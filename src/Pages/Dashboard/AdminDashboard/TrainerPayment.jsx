...
const TrainerPayment = () => {
  ...
  const { name, expectedPayment, email, _id } = data;
  const handleSubmit = async (e) => {
    ...
    const adminPaymentRes = await axiosPublic.post(
      "/adminPayments",
      completePayment
    );
    if (adminPaymentRes.data.insertedId) {
      ...
      const updatedPayment = await axiosPublic.patch(
        `/trainers/payments/${_id}`,
        payment
      );
